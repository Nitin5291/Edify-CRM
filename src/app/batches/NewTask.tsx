...
import { Prioritydata } from "@/CommonData";
import { getTaskData } from "@/lib/features/task/taskSlice";
...
const NewTask = ({ name, id }: { name?: string, id?: number }) => {
  ...
  console.log("🚀 ~ NewTask ~ taskData:", taskData)

  const OwnerData: HostItem[] = allUser?.users?.map((item: any) => {
    return { lable: item?.user_metadata?.name, value: item?.id };
  });

  useEffect(() => {
    dispatch(getUser());
    dispatch(getTaskData(`${name}=${id}`));
  }, [id]);
