...
import LeadeFields from "./LeadeFields";
...
const initialColumnDefs: ColDef[] = [
  ...
  },
  {
    ...
    cellRenderer: (params: { data: any }) => {
      const data = params.data;
      const statusStyle =
        // data?.leadValue === "Cold Lead" ? { color: "red" } : {};
        data?.leadStatus === "Cold Lead" ? { color: "red" } : {};

      return (
        <div style={statusStyle}>
          {/* {data?.leadValue ? data?.leadValue : ""} */}
...

  {
    ...
    headerClass: 'center-header', // Custom class for header alignment
  },
  {
    field: "phone",
    headerName: "Phone",
...
const LeadesPage = ({
  ...
  // const leadActiveFilter = lead?.kanbanAllData?.data?.map((item: any) => {
  //   return { key: item.kanbanHeaders, value: item.id }
  // })
    ...
    if (data === "my_leads") {
      ...
      const filterData = `fromDate=${moment()
        ...
        .format("YYYY-MM-DD")}&toDate=${moment()
          ...
          .format("YYYY-MM-DD")}`;
      dispatch(getLeadFilter({ leadStage: "lead", data: filterData }));
    } else if (data === "todays_expected_walkins") {
      const filterData = `todayWalk=${data}`;
      dispatch(getLeadFilter({ leadStage: "lead", data: filterData }));
      ...
      if (searchValue) {
        return dataFilter(
          filtetData?.map((item: any) => ({
            // ...item, leadValue: leadActiveFilter?.filter((i: any) => i?.value == item?.leadStatus)?.[0]?.key
            ...item,
          })),
          searchValue,
          ["name", "techStack", "phone", "classMode", "leadStatus", 'courseList', "leadValue"]
        );
      } else {
        return filtetData?.map((item: any) => ({
          // ...item, leadValue: leadActiveFilter?.filter((i: any) => i?.value == item?.leadStatus)?.[0]?.key
          ...item
        }));
      }
  ...
  const exportOnClick = () => {
    const data = filteredActivities?.map((item: any) => {
      return {
        'Name': item?.name,
        'Email': item?.email,
        "Expected Registration date": item?.expRegistrationDate && moment(item?.expRegistrationDate).format("DD-MM-YYYY"),
        "Next FollowUp date": item?.nextFollowUp && moment(item?.nextFollowUp).format("DD-MM-YYYY"),
        "Demo Attended date": item?.demoAttendedDate && moment(item?.demoAttendedDate).format("DD-MM-YYYY"),
        'Phone': item?.phone,
        'Price List': item?.priceList,
        'Fee Quoted': item?.feeQuoted,
        'Lead Source': item?.leadSource,
        'Type': item?.type,
        'Lead Score': item?.leadScore,
        "Course List": item?.courseList,
        "Class Mode": item?.classMode,
        "Tech Stack": item?.techStack,
        "Batch Timing": item?.batchTiming,
        "Lead Owner": item?.leadOwner,
        "Cold Lead Reason": item?.coldLeadReason,
        "Lead Status": item?.leadStatus,
        "Warm Stage": item?.warmStage,
        "Visited Date": item?.visitedDate && moment(item?.visitedDate).format("DD-MM-YYYY"),
        "Expected walk-in date": item?.expectedWalkInDate && moment(item?.expectedWalkInDate).format("DD-MM-YYYY"),
        "Amount Spent On Campaign": item?.amountSpent,
            ...
            >
              {lead?.isLoader ? (
                ...
                <div
                  ...
                  style={{ width: "100%" }}
                >
                  {filteredActivities && <AgGridReact
                    ref={tableRef}
